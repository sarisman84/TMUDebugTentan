...
PauseMenu::PauseMenu(void) {
	...
	const std::string resume = "RESUME BUGGY GAME";
	const std::string options = "OPTIONS";
	const std::string quitMenu = "QUIT TO MENU";
	const std::string quitDekstop = "QUIT TO DESKTOP";

	this->lMO.push_back(new MenuOption(resume.c_str(), 0, 156));
	this->lMO.push_back(new MenuOption(options.c_str(), 0, 180));

	this->lMO.push_back(new MenuOption(quitMenu.c_str(), 0, 228));
	this->lMO.push_back(new MenuOption(quitDekstop.c_str(), 0, 252));
